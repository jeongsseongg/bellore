  ...
  return `<button type="button" class="shipping-address-popup__backdrop" data-address-dismiss aria-label="배송지 관리 닫기"></button>
    <section class="shipping-address-popup__sheet" role="dialog" aria-modal="true" aria-labelledby="shippingAddressPopupTitle" tabindex="-1">
      <header class="shipping-address-popup__header">
        ...
        <div><p data-address-eyebrow>배송 정보</p><h2 id="shippingAddressPopupTitle" data-address-title>배송지 관리</h2></div>
...
export function createShippingAddressPopup({ document: doc, window: win, max = 10, formatPhone, onUse, onSave, onDelete, onDefault, newAddress }) {
  ...
  function finishAddressSearch(data) {
    const selected = {
      ...searchSeed,
    ...
    root.classList.remove('is-searching');
    fields.postcode.value = selected.postcode;
    fields.addr1.value = selected.addr1;
    fields.addr2.value = '';
    searchView.hidden = true;
    editor.hidden = false;
    title.textContent = fields.id.value ? '배송지 수정' : '새 배송지';
    eyebrow.textContent = '배송 정보';
    fields.addr2.focus();
  }

  function showSearch(mode = 'editor', initial = {}) {
    if (!win.daum?.Postcode) {
      announce('주소 검색을 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.', true);
      return false;
    }
    searchMode = mode;
    searchSeed = mode === 'checkout' ? initial : values();
    ...
    new win.daum.Postcode({
      oncomplete: finishAddressSearch,
      onresize(size) {
        const available = Math.max(460, (Number(win.innerHeight) || 760) - 230);
        const height = Math.min(Math.max(Number(size?.height) || available, 460), available, 640);
  ...
  function openSearch(trigger, initial = {}) {
    ...
    root.hidden = false;
    doc.body.classList.add('shipping-address-popup-open');
    if (!showSearch('checkout', initial)) close();
    else sheet.focus();
  }
