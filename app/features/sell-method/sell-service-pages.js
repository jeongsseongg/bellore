...
export function initSellServicePages({ document, window, backend }) {
  ...
  root.insertAdjacentHTML('beforeend', '<div class="sell-service-action" id="sellServiceAction" hidden><section class="sell-service-action__panel" role="dialog" aria-modal="true" aria-labelledby="sellServiceActionTitle"><h2 id="sellServiceActionTitle" data-sell-action-title>거래 방법을 선택해주세요</h2><p class="sell-service-action__amount" data-sell-action-amount></p><div class="sell-service-action__methods"><button type="button" data-sell-trade="visit">방문<br>거래</button><button type="button" data-sell-trade="delivery">택배<br>거래</button><button type="button" data-sell-trade="quick">퀵<br>거래</button></div><div class="sell-service-action__buttons"><button type="button" data-sell-action-close>취소</button><button type="button" data-sell-action-confirm disabled>이 방법으로 진행</button></div></section></div>');
  ...
  function connect() {
    bell.addEventListener('click', (event) => { event.stopPropagation(); panel.hidden = !panel.hidden; });
    root.addEventListener('click', (event) => { handleClick(event); });
    document.addEventListener('click', (event) => {
      const trigger = event.target.closest('[data-sell-service-open]');
      if (trigger && !root.contains(trigger)) {
        event.preventDefault();
        open(trigger, trigger.dataset.sellServiceOpen, trigger.dataset.sellServiceId);
        return;
      }
      if (!panel.hidden && !event.target.closest('#sellServiceNoticePanel') && !event.target.closest('#sellServiceNoticeToggle')) panel.hidden = true;
    });
