if ('serviceWorker' in navigator) {
    window.addEventListener('load', function () {
        navigator.serviceWorker.register('sw.js?v=20260826-member-verification-live-v1').catch(function (err) {
...
(function () {
    ...
    /* ---- 필터 상태 ---- */
    var fBrand = 'all', fModel = '', fQuery = '', fNew = false, fGrade = '', fSort = 'latest', homeRule = null;
    var cf = { sizeMin: null, sizeMax: null, priceMin: null, priceMax: null, conditionMin: null, yearMin: null, yearMax: null, warranty: '', diamond: '', colors: [], materials: [], grades: [] };
    function containsAny(text, values) { text = String(text || '').toLowerCase(); return (values || []).some(function (value) { return text.indexOf(String(value).toLowerCase()) !== -1; }); }
    ...
    function matchCard(c) {
        ...
        var bdSearch = [bd, window.BELLORE_BRAND_KR ? window.BELLORE_BRAND_KR(bd) : '', window.BELLORE_BRAND_EN ? window.BELLORE_BRAND_EN(bd) : ''].join(' ').toLowerCase();
        ...
        var hay = (bdSearch + ' ' + md + ' ' + no + ' ' + color + ' ' + mat + ' ' + pk).toLowerCase();
        ...
        if (homeRule && homeRule.maxExclusive != null && !(pr > 0 && pr < homeRule.maxExclusive)) return false;
        if (homeRule && homeRule.maxInclusive != null && !(pr > 0 && pr <= homeRule.maxInclusive)) return false;
    ...
    function applyFilters(options) {
        var keepPage = options === true;
        if (options && options.homeCategory) {
            homeRule = options; fBrand = 'all'; fModel = ''; fQuery = ''; fNew = false; fGrade = ''; fSort = options.sort || 'latest';
            cf = { sizeMin: null, sizeMax: null, priceMin: options.min == null ? null : Number(options.min), priceMax: null, conditionMin: options.gradeMin == null ? null : Number(options.gradeMin), yearMin: null, yearMax: null, warranty: '', diamond: '', colors: [], materials: [], grades: [] };
            brands.forEach(function (button) { button.classList.toggle('active', button.dataset.brand === fBrand); });
        ...
        paintPage();
        if (totalCountEl) totalCountEl.textContent = grid.querySelectorAll('.hcard').length;
        if (filteredCountEl) filteredCountEl.hidden = shown === grid.querySelectorAll('.hcard').length;
    ...
    if (brandWrap) brandWrap.addEventListener('click', function (event) {
            var btn = event.target.closest('.cat-brand');
            if (!btn || !brandWrap.contains(btn)) return;
            if (window.BELLORE_hideSearchEmpty) window.BELLORE_hideSearchEmpty();
            brands.forEach(function (x) { x.classList.remove('active'); });
            btn.classList.add('active');
            homeRule = null; fBrand = btn.dataset.brand; fModel = '';
            renderModelChips(fBrand);
            applyFilters();
            hideSuggest();
    ...
    (function () {
        ...
        function runSearch(raw) {
            ...
            homeRule = null; fBrand = 'all'; fModel = ''; fQuery = v ? aliasExpand(v) : '';
            brands.forEach(function (button) { button.classList.toggle('active', button.dataset.brand === 'all'); });
    ...
    if (catModels) catModels.addEventListener('click', function (e) {
        ...
        homeRule = null; fModel = cm.dataset.model || '';
        applyFilters();
    });
    ...
    function resetFilterModal() {
        cf = { sizeMin: null, sizeMax: null, priceMin: null, priceMax: null, conditionMin: null, yearMin: null, yearMax: null, warranty: '', diamond: '', colors: [], materials: [], grades: [] };
