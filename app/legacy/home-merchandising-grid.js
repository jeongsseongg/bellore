...
export function initLegacyHomeMerchandisingGrid({ document: doc, window: win }) {
  ...
  function apply() {
    ...
    const unused = cards.filter((card) => !used.has(card));
    ordered.forEach((card) => { card.hidden = false; });
    unused.forEach((card) => { card.hidden = true; });
    const allCards = ordered.concat(unused);
    if (allCards.some((card, index) => cards[index] !== card)) {
  ...
  return {
    update(items, audit) {
      rankedItems = items || [];
      rankedIds = rankedItems.map((item) => String(item.id));
      algorithmVersion = audit && audit.algorithm_version || 'fallback-latest';
      const legacyRows = rankedItems.map((item) => item.legacyRow).filter(Boolean);
      if (grid && legacyRows.length) {
        grid._belloreRecommendedRows = legacyRows;
        grid.dispatchEvent(new win.CustomEvent('bellore:home-recommendations', { detail: { rows: legacyRows } }));
      }
      schedule();
    },
  };
