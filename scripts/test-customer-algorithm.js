...
const paymentsUrl = html.match(/src=["'](payments\.js\?v=[^"']+)["']/)?.[1];
assert(paymentsUrl, 'payments runtime must carry a cache key');
assert(serviceWorker.includes(`'./${paymentsUrl}'`), 'service worker must precache the exact payments runtime');
