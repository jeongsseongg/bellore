...
async function validateDeployConfig() {
  ...
  assert.equal((pagesWorkflow.match(/vars\.PRODUCTION_DEPLOY_ENABLED == 'true'/g) || []).length, 3,
    'Pages verify, build와 deploy는 운영 변수가 정확히 true이거나 잠금형 SHA가 있을 때만 열려야 합니다.');
  ...
  assert.match(pagesWorkflow, /verify:\s*[\s\S]*?run:\s*node scripts\/check\.mjs/, 'Pages verify job은 전체 프로젝트 게이트를 실행해야 합니다.');
  assert.match(pagesWorkflow, /build:\s*[\s\S]*?needs:\s*\[truth_guard,\s*verify\]/,
    'Pages build job은 Truth Guard와 전체 artifact 검증을 모두 먼저 통과해야 합니다.');
...
async function validateArtifact(site, { expectSeo }) {
  ...
  assert.match(mypageShell, /<link rel="canonical" href="https:\/\/bellore\.co\.kr\/pages\/mypage\/">/,
    '마이페이지 앱 셸 canonical은 GitHub Pages 디렉터리 URL이어야 합니다.');
