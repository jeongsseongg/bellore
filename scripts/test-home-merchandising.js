...
const engine = require(path.join(root, 'app', 'legacy', 'recommendation-engine.js'));
...
assert(first.weeklySpecial.items.length > 0 && first.weeklySpecial.items.length <= 8);
...
assert.equal(manual.weeklySpecial.items[0].id, 'watch-2');
assert.equal(manual.weeklySpecial.audit.manual_selected_count, 1);
assert.equal(
  manual.recommended.items.some((item) => item.id === 'watch-2'),
  false,
  '현재 할인 중인 수동 선택 상품은 추천 영역에 중복되면 안 됩니다.'
);
assert.equal(manual.weeklySpecial.items.some((item) => item.id === 'watch-16'), false,
  '할인이 끝난 상품은 수동 선택돼 있어도 특별가 영역에 남으면 안 됩니다.');

const noSale = feature.update(catalog.map((item) => ({ ...item, saleActive: false, listPrice: item.price })));
assert.equal(noSale.weeklySpecial.items.length, 0);
assert.equal(
