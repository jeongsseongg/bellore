...
assert.match(supabaseClient, /function paymentAccessToken\(\)/);
assert.match(supabaseClient, /fetch\(PAY\.checkoutUrl/);
assert.match(checkout, /function sanitizeAttribution/);
assert.doesNotMatch(checkout, /recommendation:\s*recommendation/);
assert.match(confirmPayment, /function sanitizeAttribution/);
assert.doesNotMatch(confirmPayment, /validatedRecommendation/);
assert.match(confirmPayment, /sha256Hex\(checkoutToken\)/);
assert.match(confirmPayment, /callerId !== order\.customer_id/);
assert.match(confirmPayment, /finalize_paid_order_v2/);
