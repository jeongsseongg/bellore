...
assert.match(html, /supabase-config\.js\?v=20260827-checkout-methods-v1/, 'HTML must request the live payment config release');
assert.match(serviceWorker, /\.\/supabase-config\.js\?v=20260827-checkout-methods-v1/, 'service worker must precache the exact live payment config release');
...
for (const method of [
  ...
  assert.match(config, new RegExp(`id:\\s*"${method[0]}"[^\\n]*payMethod:\\s*"${method[1]}"[^\\n]*channelKey:\\s*"${expectedLiveChannel}"`));
}
assert.match(config, /easyPayProvider:\s*"KAKAOPAY"/);
assert.match(config, /easyPayProvider:\s*"TOSSPAY"/);

console.log('PortOne live payment channel contract: 10/10 passed');
