...
const existing = migration.indexOf('where checkout_request_key_hash = p_checkout_request_key_hash', lock);
const replay = migration.indexOf("'checkoutReplayed',true", existing);
const conflict = migration.indexOf("jsonb_build_object('error','checkout_request_conflict')", replay);
const rate = migration.indexOf('perform public.consume_checkout_rate_limit(p_rate_key);', conflict);
...
for (const marker of [
  ...
  'claimed.reserved_order_id = v_existing.id',
]) assert(migration.includes(marker), `safe retry match is missing: ${marker}`);
