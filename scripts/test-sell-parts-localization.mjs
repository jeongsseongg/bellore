...
const scriptUrl = index.match(/<script src="(script\.js\?v=[^"]+)"/)?.[1];
...
assert.ok(scriptUrl, 'the main script release URL must be present');
assert.ok(worker.includes(`'./${scriptUrl}'`), 'the service worker must cache the exact main script release URL');
