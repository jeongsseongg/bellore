...
import { verifyOtpChallenge } from "../_shared/phone-otp.mjs";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const code = safeText((error as Error)?.message, 80) ?? "OTP_VERIFY_FAILED";
    const status = code === "OTP_INVALID" ? 401 : code === "OTP_EXPIRED" ? 410 : 502;
    console.error(JSON.stringify({ event: "phone_otp_verify_failed", traceId, code }));
